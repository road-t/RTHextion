    ...
    _tb(tb)
{
    ...
    // HEX column 30% narrower, Value column stretches
    ...
    ui->twTable->setColumnWidth(0, 50);  // reduced from ~70 to ~50 (30% narrower)
}
...
void TableEditDialog::showEvent(QShowEvent *ev)
{
    ...
    // Enforce HEX column width to stay at 50 pixels (30% narrower)
    // Must be done after rows are added to prevent ResizeToContents from overriding
    ui->twTable->setColumnWidth(0, 50);
}
...
void TableEditDialog::on_pbAdd_clicked()
{
    ...
    if (auto lineEdit = hexDialog.findChild<QLineEdit*>()) {
        auto *validator = new QRegularExpressionValidator(
            QRegularExpression(QStringLiteral("^([0-9A-Fa-f]{2})+$")), lineEdit);
    ...
    // Validate with regexp: must be one or more complete hex byte pairs
    static const QRegularExpression hexBytesRe(QStringLiteral("^([0-9A-F]{2})+$"));
    if (!hexBytesRe.match(input).hasMatch())
    {
